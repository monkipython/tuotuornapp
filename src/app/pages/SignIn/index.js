...
class SignIn extends React.Component {
  ...
  render() {
    let { signUp, forgotPass, goBack, type } = this.props
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.text}>{__.t("Do you have account with TuoTuo?")} <Text style={styles.signUp} onPress={signUp}>{__.t('Sign Up')}</Text></Text>
          <Text style={styles.text}>{__.t("Forgot your password?")} <Text style={styles.forgotPass} onPress={forgotPass}>{__.t('Recover Password')}</Text></Text>
        </View>
    ...
    LoginManager.logInWithReadPermissions(['public_profile', 'email']).then(
      function (result) {
        if (result.isCancelled) {
          //Alert.alert(__.t('Alert'), __.t('Login was cancelled'));
        } else {
        ...
        Alert.alert(__.t('Alert'), 'Login failed with error: ' + error);
      }
    );
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.type == ActionTypes.SIGN_IN_FAIL && this.isLogging == true) {
      ...
      Alert.alert(__.t('Alert'), nextProps.message)
    }
    ...
    if (nextProps.type == ActionTypes.SIGN_IN_SUCCESS && this.isLogging == true) {
      ...
      OneSignal.setEmail(email, (error) => {
